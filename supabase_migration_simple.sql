-- Simple migration - just create everything fresh
-- This avoids issues with dropping non-existent objects

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create custom types (will fail if they exist, but that's OK)
DO $$
BEGIN
...
EXCEPTION WHEN duplicate_object THEN
    RAISE NOTICE 'point_status type already exists';
END $$;
...
EXCEPTION WHEN duplicate_object THEN
    RAISE NOTICE 'action_type type already exists';
END $$;
...
EXCEPTION WHEN duplicate_object THEN
    RAISE NOTICE 'notification_type type already exists';
END $$;
...
-- Red Points table
CREATE TABLE IF NOT EXISTS public.red_points (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_red_points_status ON public.red_points(status);
CREATE INDEX IF NOT EXISTS idx_red_points_department ON public.red_points(department_id);
CREATE INDEX IF NOT EXISTS idx_red_points_point_number ON public.red_points(point_number);
CREATE INDEX IF NOT EXISTS idx_status_history_point ON public.status_history(point_id);
CREATE INDEX IF NOT EXISTS idx_status_history_timestamp ON public.status_history(timestamp DESC);
...
-- Enable Realtime for all tables (create publication if it doesn't exist)
DO $$
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Could not set up realtime publication: %', SQLERRM;
END $$;

-- Insert test departments (only if they don't exist)
INSERT INTO public.departments (name, location) 
SELECT 'Lager A', 'Byggnad 1'
WHERE NOT EXISTS (SELECT 1 FROM public.departments WHERE name = 'Lager A');

INSERT INTO public.departments (name, location) 
SELECT 'Lager B', 'Byggnad 2'
WHERE NOT EXISTS (SELECT 1 FROM public.departments WHERE name = 'Lager B');

INSERT INTO public.departments (name, location) 
SELECT 'Lager C', 'Byggnad 3'
WHERE NOT EXISTS (SELECT 1 FROM public.departments WHERE name = 'Lager C');

-- Generate 60 red points (only if they don't exist)
DO $$
DECLARE
  dept_ids UUID[];
  dept_id UUID;
...
BEGIN
  ...
  SELECT COUNT(*) INTO existing_count FROM public.red_points;
  
  -- Only generate if we don't have 60 points
  IF existing_count < 60 THEN
    SELECT ARRAY_AGG(id) INTO dept_ids FROM public.departments;
    
    FOR i IN 1..60 LOOP
      -- Skip if this point already exists
      IF NOT EXISTS (SELECT 1 FROM public.red_points WHERE point_number = i) THEN
